...
<main class="ttr-wrapper">
	<div class="container-fluid">
		...
		if (isset($_GET['id'])) {
			$id = $_GET['id'];
			$select_query = "SELECT * FROM course INNER JOIN course_category ON course.course_category_id = course_category.category_id WHERE course.id=$id";
			$show_result = mysqli_query($conn, $select_query);
		...
		if (isset($_POST['submit'])) {
			...
			if ($course_category_id != "" && $title != "" && $content != "" && $img != "") {
				...
				$update_query = "UPDATE course SET course_category_id='$course_category_id', title='$title', content='$content', img='$img' WHERE id=$id";
				$update_result = mysqli_query($conn, $update_query);
				if ($update_result) {
		?>
					<meta http-equiv="refresh" content="0,url=managecourse.php?msg=updatecoursesuccess">

		<?php
				} else {
					echo "Course couldn't be updated.";
				}
			} else {
						...
						<form class="edit-profile m-b30" form action="#" method="POST" enctype="multipart/form-data">
							<div class="row">
								<div class="form-group col-12">
									...
									<select name="cat_id">
										<option value="<?php echo $show_row['category_id']; ?>"><?php echo $show_row['name']; ?></option>
										<?php
										$select_query = "SELECT * FROM course_category";
										$select_result = mysqli_query($conn, $select_query);
										while ($data = mysqli_fetch_array($select_result)) {
											if ($show_row['name'] != $data['name']) {
										?>
								...
								<div class="form-group col-12">
									...
									<div>
										<textarea class="form-control" name="content" id="content" rows="3"><?php echo $show_row['content']; ?></textarea>
									</div>
								</div>
								...
								<div class="input-group mb-5 col-12">
									<input id="imagebox" type="text" class="form-control" value="<?php echo $show_row['img']; ?>" name="img" readonly>
